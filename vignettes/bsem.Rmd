...
for (i in 1:Nv) {
  errors[i, ] <- rnorm(Ne, 0, sqrt(sigma2[i]))
}
```
...
for (k in 1:K) {
  alpha[B[[k]], k] <- runif(lengths(B)[k], 0.5, 2) * sample(c(-1, 1), lengths(B)[k], replace = T)
}
...
gridExtra::grid.arrange(
  ...
  bsem::arrayplot(multifact$mean_loadings,
    main = "estimates",
    mini = -2, maxi = 2
  ),
...
ggplot(aes(y = find, x = m, xmin = li, xmax = lu), data = dt) +
  ...
  theme_classic() +
  labs(
    title = paste("Latent variable", colnames(sem$mean_loadings)[3]),
