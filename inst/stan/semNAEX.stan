...
model{
  ...
  for(i in 1:Nob){
    X[idob[i,1],idob[i,2]] ~ normal(alpha[asc[i]-Nex,] * lambda[,idob[i,2]], sqrt(sigma2[asc[i]]));
  }
 ...
 // independet lambda prior
  ...
  // exogenous regression
   X[idex[1],] ~ normal(gamma0[1] + gamma[1:ngamma[1]]*lambda[idlambex[1:ngamma[1]],], tau2[1]);

  if(Nex>1){
    for(i in 2:Nex){
     X[idex[i],] ~ normal(gamma0[i] + gamma[(sum(ngamma[1:(i-1)])+1):sum(ngamma[1:i])]*lambda[idlambex[(sum(ngamma[1:(i-1)])+1):sum(ngamma[1:i])],], tau2[i]);
    }
  }
